...
// Rota de setup único — protegida por SETUP_SECRET env var
// Após rodar com sucesso, remova esta rota ou desative via env
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (existingGroup) {
      return NextResponse.json({ message: 'Setup já foi executado anteriormente.', alreadyDone: true })
    }
    ...
    await prisma.unit.create({
      data: { brandId: marcaBurger.id, name: 'Smash Burger - Pinheiros', code: 'SB-002', city: 'São Paulo', state: 'SP' },
    })
    await prisma.unit.create({
      data: { brandId: marcaPizza.id, name: 'La Pizza - Moema', code: 'LP-001', city: 'São Paulo', state: 'SP' },
    })
    await prisma.unit.create({
      data: { brandId: marcaCafe.id, name: 'Café Verde - Jardins', code: 'CV-001', city: 'São Paulo', state: 'SP' },
    })
    ...
    await prisma.user.create({
      data: { email: 'admin@grupogourmet.com.br', passwordHash: senhaAdmin, name: 'Administrador Grupo', role: UserRole.GROUP_ADMIN, groupId: grupo.id },
    })
    await prisma.user.create({
      data: { email: 'gerente.burger@grupogourmet.com.br', passwordHash: senhaGerente, name: 'Gerente - Smash Burger', role: UserRole.BRAND_ADMIN, groupId: grupo.id, brandId: marcaBurger.id },
    })
    await prisma.user.create({
      data: { email: 'estoquista.sb001@grupogourmet.com.br', passwordHash: senhaEstoque, name: 'Estoquista - Vila Madalena', role: UserRole.STOCK_KEEPER, groupId: grupo.id, brandId: marcaBurger.id, unitId: unidadeBurger1.id },
    })
    ...
    // ── Produtos ─────────────────────────────────────────────
    const produtos = [
      { sku: 'CARNE-180G', internalCode: 'MP-001', name: 'Carne Bovina Moída 180g', categoryId: catCarnes.id, unitOfMeasure: UOM.KG, minStock: 10, idealStock: 50, shelfLifeDays: 3 },
      { sku: 'PAO-BRIOCHE', internalCode: 'MP-002', name: 'Pão Brioche para Hambúrguer', categoryId: catPaes.id, unitOfMeasure: UOM.UN, minStock: 50, idealStock: 200, shelfLifeDays: 5 },
      { sku: 'QUEIJO-CHED', internalCode: 'MP-003', name: 'Queijo Cheddar Fatiado', categoryId: catLaticinios.id, unitOfMeasure: UOM.KG, minStock: 2, idealStock: 10, shelfLifeDays: 14 },
      { sku: 'ALFACE-CRE', internalCode: 'MP-004', name: 'Alface Crespa', categoryId: catHortifruti.id, unitOfMeasure: UOM.KG, minStock: 2, idealStock: 8, shelfLifeDays: 3 },
      { sku: 'TOMATE-ITA', internalCode: 'MP-005', name: 'Tomate Italiano', categoryId: catHortifruti.id, unitOfMeasure: UOM.KG, minStock: 2, idealStock: 10, shelfLifeDays: 5 },
      { sku: 'BACON-FATI', internalCode: 'MP-006', name: 'Bacon Fatiado', categoryId: catCarnes.id, unitOfMeasure: UOM.KG, minStock: 2, idealStock: 8, shelfLifeDays: 7 },
      { sku: 'MOLHO-ESP', internalCode: 'MP-007', name: 'Molho Especial da Casa', categoryId: catLaticinios.id, unitOfMeasure: UOM.L, minStock: 1, idealStock: 5, shelfLifeDays: 7 },
      { sku: 'REFRI-350', internalCode: 'BV-001', name: 'Refrigerante Lata 350ml', categoryId: catBebidas.id, unitOfMeasure: UOM.UN, minStock: 24, idealStock: 120, shelfLifeDays: 180 },
      { sku: 'AGUA-500', internalCode: 'BV-002', name: 'Água Mineral 500ml', categoryId: catBebidas.id, unitOfMeasure: UOM.UN, minStock: 12, idealStock: 60, shelfLifeDays: 365 },
      { sku: 'EMBAL-BURG', internalCode: 'DC-001', name: 'Embalagem Hambúrguer', categoryId: catDescartaveis.id, unitOfMeasure: UOM.UN, minStock: 100, idealStock: 500 },
    ]
    ...
    for (const lote of lotes) {
      ...
      await prisma.stockMovement.create({
        data: { productId, unitId: unidadeBurger1.id, lotId: novoLote.id, userId: adminUser.id, type: 'ENTRY', quantity: lote.qty, unitCost: lote.cost, totalCost: lote.qty * lote.cost, reason: 'Estoque inicial (setup)' },
      })
    ...
    await prisma.technicalSheet.create({
      data: {
        ...
        yield: 1, yieldUnit: 'UN', preparationTime: 8, cost: 0,
        ingredients: {
          create: [
    ...
    await prisma.alert.createMany({
      data: [
        { type: 'EXPIRING_PRODUCT', unitId: unidadeBurger1.id, productId: produtosCriados['CARNE-180G'], message: 'Carne Bovina Moída vence em 3 dias (Lote LT-CARNE-001)', severity: 'HIGH', read: false },
        { type: 'EXPIRING_PRODUCT', unitId: unidadeBurger1.id, productId: produtosCriados['ALFACE-CRE'], message: 'Alface Crespa vence em 3 dias (Lote LT-ALFACE-001)', severity: 'MEDIUM', read: false },
